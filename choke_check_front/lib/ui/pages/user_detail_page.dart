...
class _UserDetailPageState extends State<UserDetailPage> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider.value(
        value: _userBloc..add(FetchUserDetailEvent(username: widget.username)),
        child: Scaffold(
          appBar: AppBar(
            leading: InkWell(
              onTap: () {
      ...
      child: BlocConsumer<PostBloc, PostState>(
        ...
        listener: (context, state) {
          if (state is AddLikeSuccess) {
            _userBloc.add(FetchUserDetailEvent(username: widget.username));
  ...
  _getDeleteButton(Post post) {
    return FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            if (post.authorName! == snapshot.data) {
              return BlocProvider.value(
                value: _postBloc,
                child: BlocConsumer<PostBloc, PostState>(
